...
/**
 * ForgeCRM — Edit Automation Rule Page
 *
 * Pre-populates the workflow builder with existing rule parameters,
 * conditions, and actions, and submits updates via updateAutomationRule mutation.
 */
...
import {
  ...
  Save, CheckCircle2, XCircle, Activity, User, Building2,
  TrendingUp, CheckSquare2, Users, Bell, Globe, Archive,
  ListChecks, RefreshCw, Loader2,
} from 'lucide-react';
...
import type {
  ...
  ConditionOperator, TriggerEvent,
} from '@/types';
...
function ActionConfigForm({
  ...
  return <p className="text-xs text-[#65656e]">No additional configuration required.</p>;
}

// ── Inner Form Component ──────────────────────────────────────────────────────

function EditWorkflowInner() {
  const params = useParams();
  const router = useRouter();
  const ruleId = params.id as string;

  const { useAutomationRule, updateAutomationRule } = useAutomation();
  ...
  // Pre-populate state when rule loads
  useEffect(() => {
    if (rule) {
      setName(rule.name);
      setDescription(rule.description ?? '');
      setSelectedTrigger(rule.trigger_event as TriggerEvent);
      setConditionLogic((rule.condition_logic as 'AND' | 'OR') ?? 'AND');
      setConditions(
  ...
  const handleSave = () => {
    ...
    updateAutomationRule.mutate(
      ...
      },
      {
        onSuccess: () => {
          setSaved(true);
          setTimeout(() => router.push(`/automations/${ruleId}`), 800);
        },
      },
    );
  };
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-bold text-[#f2f2f3]">Edit Automation</h1>
          <p className="text-sm text-[#9898a0]">Update rule trigger, conditions, and action steps.</p>
        </div>
        <button
          id="automation-update-save-btn"
          onClick={handleSave}
          disabled={updateAutomationRule.isPending || saved}
      ...
      {updateAutomationRule.isError && (
        <div className="flex items-center gap-2 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-400">
          <XCircle className="h-4 w-4 shrink-0" />
          Failed to update automation rule. Please verify form values.
        </div>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-[rgba(255,255,255,0.07)] bg-[#141416] p-4">
          <SectionHeader title="Conditions (Optional)" subtitle="Only run when these conditions pass." />
