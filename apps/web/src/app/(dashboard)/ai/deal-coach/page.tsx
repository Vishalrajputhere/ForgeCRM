...
import {
  Send, ShieldAlert, TrendingUp, Target, RefreshCw,
  Award, CheckCircle2,
} from 'lucide-react';
...
const DEAL_COACH_SUGGESTIONS: PromptSuggestion[] = [
  { icon: <TrendingUp className="h-3.5 w-3.5" />, label: 'Analyze deal health', skill: 'deal_health' },
  { icon: <Award className="h-3.5 w-3.5" />, label: 'Win probability prediction', skill: 'win_probability' },
  { icon: <ShieldAlert className="h-3.5 w-3.5" />, label: 'Detect deal risks', skill: 'risk_detection' },
  { icon: <Target className="h-3.5 w-3.5" />, label: 'Next best actions', skill: 'next_best_action' },
  { icon: <CheckCircle2 className="h-3.5 w-3.5" />, label: 'Closing readiness checklist', skill: 'closing_readiness' },
];
...
export default function AIDealCoachPage() {
  ...
  const [messages, setMessages] = React.useState<SkillMessage[]>([
    {
      id: 'welcome',
      role: 'assistant',
      content: "Welcome to Enterprise Deal Coach! I analyze pipeline health, win probability, stakeholder coverage, risks, and closing readiness.\n\nSelect a deal or ask me to:\n• Analyze deal health\n• Predict win probability\n• Detect risks & blockers\n• Recommend next best actions",
  ...
  const handleSend = async (promptText?: string, promptSkill?: string) => {
    ...
    const skillKey = promptSkill ?? 'deal_health';
    ...
    try {
      ...
      if (res.ok) {
        ...
        const assistantMsg: SkillMessage = {
          id: loadingId,
          role: 'assistant',
          content: data.summary ?? 'Deal analysis complete.',
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          skill: data.skill,
        ...
        const errMsg: SkillMessage = {
          id: loadingId,
          role: 'assistant',
          content: 'Unable to complete Deal Coach analysis. Please try again.',
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        };
      ...
      const errMsg: SkillMessage = {
        id: loadingId,
        role: 'assistant',
        content: 'Connection error calling Deal Coach API.',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
      setMessages((prev) => prev.map((m) => (m.id === loadingId ? errMsg : m)));
    } finally {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] bg-background overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 border-r border-border-subtle">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle bg-elevated/50 backdrop-blur-sm shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-rose-500/20 to-amber-500/20 border border-rose-500/30 flex items-center justify-center text-rose-400">
              <ShieldAlert className="h-4 w-4" />
            </div>
            <div>
              <h1 className="text-sm font-bold text-primary leading-none">Enterprise Deal Coach</h1>
              <p className="text-[10px] text-muted">Win Probability · Deal Health · Risk Radar · Next Best Actions</p>
            </div>
          </div>
        ...
        <PromptSuggestionBar
          suggestions={DEAL_COACH_SUGGESTIONS}
          disabled={isLoading}
        ...
        <div className="px-4 py-3 border-t border-border-subtle bg-elevated/40 backdrop-blur-sm shrink-0">
          <div className="flex items-end gap-2 bg-surface rounded-xl border border-border-default focus-within:border-accent/40 focus-within:ring-1 focus-within:ring-accent/20 transition-all shadow-sm">
            <input
              ...
              placeholder={`Ask Deal Coach about ${selectedDealName}...`}
              disabled={isLoading}
      ...
      <div className="w-80 shrink-0 hidden lg:flex flex-col bg-surface border-l border-border-subtle overflow-y-auto p-4 space-y-4">
        <DealHealthTimeline dealName={selectedDealName} healthScore={62} />
        <RiskPanel overallRiskLevel="High" />
