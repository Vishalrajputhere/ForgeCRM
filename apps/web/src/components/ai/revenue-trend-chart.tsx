...
export function RevenueTrendChart() {
  ...
  const values = [320, 350, 410, 460, 520, 580]; // $K ARR
    ...
    <div className="p-4 rounded-xl bg-surface border border-border-subtle space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-bold text-primary">ARR Revenue Trajectory ($K)</h3>
        </div>
        ...
        {values.map((v, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
              <span className="text-[9px] font-bold text-muted group-hover:text-primary transition-colors">${v}K</span>
              <div
