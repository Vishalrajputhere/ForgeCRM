"""
ForgeCRM — AI Skills Shared Prompt Templates Engine

Provides versioned, Jinja2-style prompt templates for all AI Skills.
Every skill uses PromptTemplate to enforce consistent prompt formatting,
versioning, and few-shot learning across providers.
...
class PromptTemplate:
    """Versioned prompt template with variable interpolation and few-shots."""
...
# ─────────────────────────────────────────────────────────────────────────────
# Built-in Versioned Templates for Phase 7.4 AI Skills
# ─────────────────────────────────────────────────────────────────────────────
...
MEETING_BRIEF = PromptTemplate(
    template_id="meeting_brief",
    version="1.0.0",
    description="Pre-meeting briefing document for a contact or company",
    system_prompt="""You are a sales preparation analyst for ForgeCRM.
Produce a pre-meeting brief for the upcoming meeting.
...
Rules:
- Include: Contact background, company overview, relationship history, open deals, action items from last meeting.
- Suggest 3–5 talking points.
- Note potential objections and how to handle them.
- Keep it under 400 words.""",
    user_template="Prepare a meeting brief for: {entity_name}. Meeting purpose: {purpose}.",
)
...
PIPELINE_EXPLANATION = PromptTemplate(
...
def get_template(template_id: str) -> PromptTemplate:
    """Returns a PromptTemplate by ID, raising ValueError if not found."""
